...
World::World(Game* game, State* state)
	: mSceneGraph()
	, mGame(game)
	...
	, CurrState(state)
{
	...
	mBackground = new SpriteNode(CurrState);
	mPlayerAircraft = new Aircraft(Aircraft::Eagle , CurrState);
...
{
	// // Player
	std::unique_ptr<Aircraft> player(new Aircraft(Aircraft::Eagle, CurrState));
	...
	// Enemy
	std::unique_ptr<Aircraft> enemy3(new Aircraft(Aircraft::Raptor, CurrState));
	...
	// Background
	std::unique_ptr<SpriteNode> backgroundSprite(new SpriteNode(CurrState));
	...
	// // title
	// std::unique_ptr<TitleSprite> title(new TitleSprite(CurrState));
